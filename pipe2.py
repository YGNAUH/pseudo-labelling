...
def write_result(time_used,epoch_loss_list,model1_iou,
  ...
  save_path = "./pseudo_more/" + str(image1Folder) + "ratio" + str(pseudo_ratio) + "/"
...
def plotAU(AUloader):
    ...
    for i, batch in enumerate(AUloader):
        ...
        save_name = "./pseudo_more/20ratio" + str(pseudo_ratio) + "/AU" + str(i) + ".tif"
...
while num_folder_label < folder_label_range:
    ...
    pseudolabels = np.empty((len(pseudoSet),1,1,512,1024))
    ...
    for i, batch in enumerate(pseudoLoader):
        ...
        output_tensor = model_label(input_img).cpu()

        output_label = output_tensor.detach().numpy()
        output_label= np.where(output_label > 0.9, 1, output_label)
        output_label= np.where(output_label < 0.1, 0, output_label)
        output_label = np.where((output_label > 0 )&(output_label<1), None, output_label)
        #print(output_label)
        ...
        output_binary = binaryMask(output_tensor)
        pseudo_acc += float(acc_fcn(output_binary,target))

        
        if ((i+1)%sv_image == 0):
            
            pred = mask2graph(output_binary).view(512,1024)
    ...
    save_file = "./pseudo_more/" + str(image1Folder) + "ratio" + str(pseudo_ratio) + "/"
    ...
    for epoch in range(num_epoch):
        ...
        for i, batch in enumerate(pseudoLoader):
            ...
            output_np = (output_label).cpu().detach().numpy()
            target_graph = pseudolabels[i]
            
            alpha = 0.3*1025*512/(len(target_graph[np.isnan(target_graph)]))
            #print(alpha)
            target_graph = np.where(np.isnan(target_graph), output_np, target_graph)
           
            
            target_label = torch.tensor(target_graph).to(device)


            loss = alpha*loss_function(output_label.to(torch.float32),target_label.to(torch.float32))
            loss.backward()
            optimizer_label.step()
            running_loss += loss.item()
        ...
        pseudolabels = np.zeros((len(pseudoSet),1,1,512,1024))
        for i, batch in enumerate(pseudoLoader):
            input_img = batch[0].to(device)

            output_label = model_label(input_img).cpu()
            output_cp = output_label
            output_label = output_label.cpu().detach().numpy()
            output_label= np.where(output_label > 0.9, 1, output_label)
            output_label= np.where(output_label < 0.1, 0, output_label)
            output_label = np.where((output_label > 0)&(output_label<1), None, output_label)
            ...
            output_cp = binaryMask(output_cp)
            target = graph2mask(batch[1])
            pseudo_acc += float(acc_fcn(output_cp,target))


            if ((i+1)%sv_image == 0) and ((epoch+1)%10 == 0):
                print("pseudo_image generating")
                pred = mask2graph(output_cp).view(512,1024)
                label = mask2graph(target.view(512,1024))
                predLines = lineDrawingPred(pred)
                labelLines = lineDrawingLabel(label)
                background = input_img.view(512,1024).cpu()
